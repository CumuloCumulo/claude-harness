...
tools = [
  ...
  Tool 2 (Read, safe)   - status: completed  ← 結果発射待ち
...
tools = [
  ...
  Tool 3 (Bash, unsafe)  - status: executing  ← まだ実行中
...
```
1. Read("src/main.ts")       — 並行安全
2. Grep("TODO")              — 並行安全
3. Read("src/utils.ts")      — 並行安全
4. Bash("npm test")          — 非並行安全
5. Edit("src/main.ts", ...)  — 非並行安全
```
...
3つの並行安全ツールが同時に実行を開始します。

**Phase 2：Bashキュー待ち**

```
t=3  addTool(Bash npm test)    → queued → processQueue() → canExecute? NO (実行中にsafeツールあり、自身はunsafe)
```

Bashツールがキュー待ち状態に入ります。

**Phase 3：Editキュー待ち**

```
t=4  addTool(Edit main.ts)     → queued → processQueue() → canExecute? NO → break (unsafeツールがブロック)
```

EditツールもキューQ待ちに入ります。キュー待ちのBash（unsafe）に遭遇し、`break` で走査が停止します。
...
最初の読み取り完了が `processQueue()` をトリガーします。この時点ですべての読み取りが完了または完了中で、Bashがキュー内の最初のキュー待ちツールです。Bashが実行可能かは他にexecutingツールがあるかに依存します。t=5の時点ですべての読み取りがちょうど完了していれば、Bashは排他実行を開始できます。

**Phase 5：結果の順序付き発射**

```
getRemainingResults() 発射:
  → Read(main.ts) 結果
  → Grep(TODO) 結果
  → Read(utils.ts) 結果
  (Bash完了待ち...)
...
```
t=8  Bash(npm test) が is_error: true を返す
     → hasErrored = true
     → siblingAbortController.abort('sibling_error')
     → Edit(src/main.ts) が executeTool() エントリでabortを検出
     → 合成エラーを生成: "Cancelled: parallel tool call Bash(npm test) errored"
```

Editツールは実行されず、モデルは2つのエラーメッセージを受け取ります。Bashの真のエラーと、Editのキャンセル通知です。モデルはこれらの情報に基づいて次のステップを決定できます。
