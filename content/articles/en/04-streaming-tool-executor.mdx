...
tools = [
  ...
  Tool 2 (Read, safe)   - status: completed  <- results pending emission
...
tools = [
  ...
  Tool 3 (Bash, unsafe)  - status: executing  <- still running
...
```
1. Read("src/main.ts")       — concurrency-safe
2. Grep("TODO")              — concurrency-safe
3. Read("src/utils.ts")      — concurrency-safe
4. Bash("npm test")          — not concurrency-safe
5. Edit("src/main.ts", ...)  — not concurrency-safe
```
...
```
t=3  addTool(Bash npm test)    -> queued -> processQueue() -> canExecute? NO (safe tools executing, but self is unsafe)
```
...
```
t=4  addTool(Edit main.ts)     -> queued -> processQueue() -> canExecute? NO -> break (unsafe tool blocks)
```
...
```
getRemainingResults() emits:
  -> Read(main.ts) result
  -> Grep(TODO) result
  -> Read(utils.ts) result
  (waiting for Bash to complete...)
  -> Bash(npm test) result
  (waiting for Edit to complete...)
  -> Edit(src/main.ts) result
```
...
```
t=8  Bash(npm test) returns is_error: true
     -> hasErrored = true
     -> siblingAbortController.abort('sibling_error')
     -> Edit(src/main.ts) detects abort at executeTool() entry
     -> Generates synthetic error: "Cancelled: parallel tool call Bash(npm test) errored"
```
