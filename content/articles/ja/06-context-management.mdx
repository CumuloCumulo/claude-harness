...
```mermaid
flowchart TD
    A["tokenCountWithEstimation(messages)"] --> B{"最後の assistant に\nusage データあり？"}
    B -->|はい| C["API usage から\n精確なトークン数を取得"]
    B -->|いいえ| D["全メッセージに対して\n粗い推定を実行"]
    C --> E["usage 以降の\n新メッセージを粗く推定"]
    E --> F["精確数 + 粗い増分\n= 現在のトークン合計"]
...
```mermaid
flowchart TD
    A["autoCompactIfNeeded\n各 query loop で呼び出し"] --> B{"DISABLE_COMPACT\n環境変数？"}
    B -->|はい| Z["wasCompacted: false を返す"]
    B -->|いいえ| C{"連続失敗回数\n>= 3？"}
    C -->|はい| Z
    C -->|いいえ| D{"shouldAutoCompact\ntoken > 閾値？"}
    D -->|いいえ| Z
    D -->|はい| E["Session Memory\nCompaction を試行"]
    E -->|成功| F["wasCompacted: true を返す\n失敗カウントをリセット"]
    E -->|失敗/null| G["完全な\ncompactConversation を実行"]
    G -->|成功| F
    G -->|例外| H["失敗カウントを増加\nwasCompacted: false を返す"]
...
```mermaid
flowchart TD
    A["各 API リクエスト前"] --> B["microcompactMessages\n第 1 層：軽量クリーニング"]
    B --> C{"時間間隔 >\n閾値？"}
    C -->|はい| D["Time-based MC\n古いツール結果内容をクリア"]
    C -->|いいえ| E{"Cached MC\n利用可能？"}
    E -->|はい| F["Cached MC\ncache_edits で削除"]
    E -->|いいえ| G["microcompact をスキップ"]

    H["各 query loop イテレーション"] --> I["shouldAutoCompact\ntoken が閾値超え？"]
    I -->|いいえ| J["通常の会話を続行"]
    I -->|はい| K["trySessionMemoryCompaction\n第 2 層：API 呼び出しゼロの圧縮"]
    K -->|成功| L["Session Memory で\n古いメッセージを置換"]
    K -->|失敗| M["compactConversation\n第 3 層：LLM 完全サマリー"]
    M -->|成功| N["LLM サマリーで\n古いメッセージを置換"]
    M -->|失敗| O["失敗を記録\n次回リトライまたはサーキットブレーク"]

    P["ユーザーが /compact を入力"] --> Q["compactConversation\n手動トリガーによる完全圧縮"]
