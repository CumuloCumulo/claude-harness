...
```mermaid
flowchart TD
    A["tokenCountWithEstimation(messages)"] --> B{"Does the last assistant\nmessage have usage data?"}
    B -->|Yes| C["Get API usage:\nprecise token count"]
    B -->|No| D["Run rough estimation\non all messages"]
    C --> E["Run rough estimation\non new messages after usage"]
    E --> F["Precise count + rough increment\n= current token total"]
    D --> F
...
```mermaid
flowchart TD
    A["autoCompactIfNeeded\nCalled each query loop"] --> B{"DISABLE_COMPACT\nenv variable?"}
    B -->|Yes| Z["Return wasCompacted: false"]
    B -->|No| C{"Consecutive failures\n>= 3?"}
    C -->|Yes| Z
    C -->|No| D{"shouldAutoCompact\ntokens > threshold?"}
    D -->|No| Z
    D -->|Yes| E["Try Session Memory\nCompaction"]
    E -->|Success| F["Return wasCompacted: true\nReset failure count"]
    E -->|Failure/null| G["Execute full\ncompactConversation"]
    G -->|Success| F
    G -->|Error| H["Increment failure count\nReturn wasCompacted: false"]
...
```mermaid
flowchart TD
    A["Before each API request"] --> B["microcompactMessages\nLayer 1: Lightweight cleanup"]
    B --> C{"Time gap >\nthreshold?"}
    C -->|Yes| D["Time-based MC\nClear old tool result content"]
    C -->|No| E{"Cached MC\navailable?"}
    E -->|Yes| F["Cached MC\nDelete via cache_edits"]
    E -->|No| G["Skip microcompact"]

    H["Each query loop iteration"] --> I["shouldAutoCompact\nTokens > threshold?"]
    I -->|No| J["Continue normal conversation"]
    I -->|Yes| K["trySessionMemoryCompaction\nLayer 2: Zero API-call compression"]
    K -->|Success| L["Replace old messages\nwith Session Memory"]
    K -->|Failure| M["compactConversation\nLayer 3: Full LLM summary"]
    M -->|Success| N["Replace old messages\nwith LLM summary"]
    M -->|Failure| O["Record failure\nRetry next time or trip breaker"]

    P["User types /compact"] --> Q["compactConversation\nManual full compaction"]
